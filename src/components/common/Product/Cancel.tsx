...
const Cancel = ({ amount, period, profit, btnType }: PurchaseProps) => {
  return (
    <InfoContainer>
      <InfoItem>
        <ModalItem title="투자 금액" content={amount.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",") + '원'} isModify={false} isStock={false} />
        <ModalItem title="투자 기간" content={period + '개월'} isModify={false} isStock={false} />
        <ModalItem title="기대 수익" content={profit.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",") + '원'} isModify={false} isStock={false} />
      </InfoItem>
      <Button name="취소하기" status={btnType} onClick={() => {}} />
      <TextItem>
